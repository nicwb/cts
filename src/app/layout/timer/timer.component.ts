import {
  ...
  export class TimerComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    minorTicks: number[] = Array.from({length: 60}, (_, i) => i * 6).filter(tick => tick % 30 !== 0);
    ...
    private initializeParticles(): void {
      this.particleElements = Array.from(this.el.nativeElement.querySelectorAll('.timer-particle'));
    ...
    private animateParticle(particle: HTMLElement): void {
      ...
      this.renderer.setStyle(particle, 'transition', `transform ${duration}ms ease-in-out, opacity ${duration}ms ease-in-out`);
      ...
      setTimeout(() => {
        ...
        this.renderer.setStyle(particle, 'transform', `translate(${newX}px, ${newY}px)`);
