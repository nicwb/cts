...
export class ObjectionService {
    ...
    getTokenObjections(
        tokenId: number
    ): Observable<IapiResponce<TokenWithObjections>> {
        return this.http
            .get<
                IapiResponce<TokenWithObjections>
            >('v1/Objection/GetTokenObjections/' + tokenId)
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
                })
    ...
    setNewLocalObjection(newData: ISetNewObjection): Observable<IapiResponce> {
        return this.http
            .put<IapiResponce>('v1/Objection/NewLocalObjection', newData)
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
                })
