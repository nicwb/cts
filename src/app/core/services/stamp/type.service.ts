...
export class TypeService {
    ...
    getStampTypeList(
        ...
        return this.http
            .patch<
                IapiResponce<GetStampTypes>
            >('v1/StampMaster/StampTypeList', queryParameters)
            .pipe(
                catchError((error) => {
    ...
    addNewStampType(paylod: AddStampType): Observable<IapiResponce> {
        return this.http
            .post<IapiResponce>('v1/StampMaster/CreateStampType', paylod)
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
                })
    ...
    deleteStampType(id: Number): Observable<IapiResponce> {
        return this.http
            .delete<IapiResponce>(
                'v1/StampMaster/DeleteStampTypesById?id=' + id
            )
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
