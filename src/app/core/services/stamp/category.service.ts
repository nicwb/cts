...
export class CategoryService {
    ...
    getStampLabelCategories(
        ...
        return this.http
            .patch<
                IapiResponce<GetStampCategories>
            >('v1/StampMaster/StampCategoryList', queryParameters)
            .pipe(
                catchError((error) => {
    ...
    addNewStampCategory(payload: AddStampCategory): Observable<IapiResponce> {
        return this.http
            .post<IapiResponce>('v1/StampMaster/CreateStampCategory', payload)
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
                })
    ...
    deleteStampCategory(id: Number): Observable<IapiResponce> {
        return this.http
            .delete<IapiResponce>(
                'v1/StampMaster/DeleteStampCategoryById?id=' + id
            )
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
