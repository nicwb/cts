...
export class LabelService {
    ...
    getStampLabelList(
        ...
        return this.http
            .patch<
                IapiResponce<GetStampLabels>
            >('v1/StampMaster/StampLabelList', queryParameters)
            .pipe(
                catchError((error) => {
    ...
    addNewStampLabel(payload: AddStampLabel): Observable<IapiResponce> {
        return this.http
            .post<IapiResponce>('v1/StampMaster/CreateStampLabel', payload)
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
                })
    ...
    deleteStampLabel(id: Number): Observable<IapiResponce> {
        return this.http
            .delete<IapiResponce>(
                'v1/StampMaster/DeleteStampLabelsById?id=' + id
            )
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
