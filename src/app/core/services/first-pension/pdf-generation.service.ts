...
export class PdfGenerationService {
    ...
    generatePdf(reportData: any): void {
        ...
        const addHeader = (startY: number) => {
            doc.setFontSize(14);
            doc.text(
                'Government of West Bengal - Treasury',
            ...
            doc.setFontSize(10);
            doc.text(
                `For the Period of ${reportData.response.result.pensioner.dateOfCommencement} To ${reportData.response.result.billDate}`,
                doc.internal.pageSize.getWidth() / 2,
                startY + 24,
                { align: 'center' }
            );
            ...
            doc.setFontSize(8);
            doc.text(
                `BILL ID: ${reportData.response.result.id}`,
                10,
                startY + 30
            );
            doc.text(
                `VOUCHER NUMBER: ${reportData.response.result.treasuryVoucherNo}`,
                130,
                startY + 30
            );
            doc.text(
                `BILL DATE: ${reportData.response.result.billDate}`,
                10,
                startY + 35
            );
            doc.text(
                `VOUCHER DATE: ${reportData.response.result.treasuryVoucherDate}`,
                130,
                startY + 35
            );
            doc.text(
                `PPO ID: ${reportData.response.result.pensioner.ppoId}`,
                10,
                startY + 40
            );
            doc.text(
                `PPO NUMBER: ${reportData.response.result.pensioner.ppoNo}`,
                130,
                startY + 40
            );
            const bankName =
                reportData.bankName && reportData.bankName[0]
                    ? reportData.bankName[0].name
                    : 'Unknown Bank Name';
            doc.text(`BANK NAME: ${bankName}`, 10, startY + 45);
            doc.text(
                `BANK ACCOUNT: ${reportData.response.result.pensioner.bankAccounts[0].bankAcNo}`,
                130,
                startY + 45
            );
            doc.text(
                `BANK ADDRESS: ${reportData.branchAddress}`,
                10,
                startY + 50
            );
            doc.text(
                `COMMENCEMENT DATE: ${reportData.response.result.pensioner.dateOfCommencement}`,
                10,
                startY + 55
            );
            const hoaID =
                `${reportData.response.result.pensioner.category.primaryCategory.accountHead.MajorHead}-${reportData.response.result.pensioner.category.primaryCategory.accountHead.SubmajorHead}-${reportData.response.result.pensioner.category.primaryCategory.accountHead.MinorHead}-` +
                `${reportData.response.result.pensioner.category.primaryCategory.accountHead.PlanStatus}-${reportData.response.result.pensioner.category.primaryCategory.accountHead.SchemeHead}-${reportData.response.result.pensioner.category.primaryCategory.accountHead.VotedCharged}-` +
                `${reportData.response.result.pensioner.category.primaryCategory.accountHead.DetailHead}-${reportData.response.result.pensioner.category.primaryCategory.accountHead.SubdetailHead}`;
            doc.text(`ACCOUNT HEAD: ${hoaID}`, 130, startY + 55);
            doc.text('SANCTION ORDER NUMBER: L/P/00704/2024', 10, startY + 60);
            doc.text(
                `PENSIONER NAME: ${reportData.response.result.pensioner.pensionerName} `,
                130,
                startY + 60
            );
            doc.text(
                `CATEGORY: ${reportData.response.result.pensioner.category.categoryName} `,
                10,
                startY + 65
            );
        };
        ...
        doc.text(
            `Generated by ${reportData.response.result.preparedBy}`,
            10,
            finalY + 115
        );
        doc.text(
            `Date: ${reportData.response.result.preparedOn}`,
            10,
            finalY + 120
        );
