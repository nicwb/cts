...
export class ChequeDistributionService {
    ...
    getChqueListForDistribution(
        queryParameters: DynamicTableQueryParameters
    ): Observable<IapiResponce<DynamicTable<chequeDistributionList>>> {
        return this.http
            .patch<
                IapiResponce<DynamicTable<chequeDistributionList>>
            >('v1/Cheque/cheque-received-list', queryParameters)
            .pipe(
                catchError((error) => {
    ...
    getReceivedList(): Observable<IapiResponce> {
        return this.http
            .get<IapiResponce>('v1/Cheque/getChequeReceivedDetails')
            .pipe(
                catchError((error) => {
                    throw this.toastService.showError(error.message);
                })
