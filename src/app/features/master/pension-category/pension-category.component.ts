...
export class PensionCategoryComponent implements OnInit {
    ...
    async check_for_data() {
        ...
        if (primary && sub) {
            ...
            let responseP = await firstValueFrom(
                this.service.getPrimaryCategories()
            );
            if (responseP.result && responseP.result.data) {
                for (let i = 0; i < responseP.result.data.length; i++) {
                    if (
                        responseP.result.data[i].primaryCategoryName == primary
                    ) {
                        this.PensionForm.patchValue({
                            PrimaryCategoryId: responseP.result.data[i].id,
                        });
                        this.primary_id_select = [];
                        this.primary_id_select.push({
                            label: `${responseP.result.data[i].id}-${responseP.result.data[i].primaryCategoryName}`,
                            value: responseP.result.data[i].id,
                        });
                        this.primary_id = this.primary_id_select.find(
                            (val) =>
                                val.label ==
                                `${responseP?.result?.data?.[i].id}-${responseP?.result?.data?.[i].primaryCategoryName}`
                        );
                        break;
                    }
                }
