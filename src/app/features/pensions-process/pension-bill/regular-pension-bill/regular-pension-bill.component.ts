...
export class RegularPensionBillComponent implements OnInit {
        ...
        if (this.billPrintForm.valid) {
            ...
            try {
                ...
                const response = await firstValueFrom(
                    this.pensionRegularBillService.getAllPposForRegularBill(
                        year,
                        month
                    )
                );
                if (response?.result?.ppoList) {
                    ...
                    if (this.ppoCount > 0) {
                        ...
                        this.toastService.showSuccess(
                            `${this.ppoCount} PPO bills found.`
                        );
                    } else {
                        this.progressMessage =
                            'No PPO bills found for the selected period.';
    ...
    async onGenerate() {
        if (
            this.billPrintForm.valid &&
            this.ppoCount !== null &&
            this.ppoList.length > 0
        ) {
            ...
            try {
                for (let i = 0; i < this.ppoList.length; i++) {
                    ...
                    if (!ppo || !ppo.ppoId) {
                        console.warn(
                            `Skipping PPO with undefined ppoId at index ${i}`
                        );
                        continue;
                    }
                    ...
                    const ppoBillEntryDTO: PpoBillEntryDTO = {
                        ...
                        year: this.billPrintForm.controls[
                            'year'
                        ].value.getFullYear(), // Ensure it's a number
                        toDate: new Date().toISOString().split('T')[0],
                    };

                    try {
                        const response = await firstValueFrom(
                            this.pensionRegularBillService.saveRegularPensionBill(
                                ppoBillEntryDTO
                            )
                        );
                ...
                if (successCount > 0) {
                    this.toastService.showSuccess(
                        `${successCount} PPO bills saved successfully.`
                    );
                }

                if (failedPPOs.length > 0) {
                    this.failedMessages =
                        this.formatFailedPPOsMessage(failedPPOs);
                    this.toastService.showWarning(
                        `${failedPPOs.length} PPO bills failed to save.`
                    );
                }
                ...
                this.toastService.showError(
                    'Error occurred while saving bills'
                );
                this.failedMessages = [
                    'An unexpected error occurred while saving bills.',
                ];
            } finally {
                this.spinnerService.setSpinnerVisibility(true);
                this.isGenerating = false;
            ...
            this.toastService.showError(
                'Please fetch PPO bills before generating.'
            );
        }
    }

    private formatFailedPPOsMessage(
        failedPPOs: { ppoId: string; message: string }[]
    ): string[] {
        const maxDisplayed = 500;
        const formattedMessages = failedPPOs
            .slice(0, maxDisplayed)
            .map((failed) => `PPO ID ${failed.ppoId}: ${failed.message}`);

        if (failedPPOs.length > maxDisplayed) {
            formattedMessages.push(
                `... and ${failedPPOs.length - maxDisplayed} more`
            );
        }
