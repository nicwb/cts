...
export class RegularPensionBillPrintComponent implements OnInit {
    ...
    applyChoiceValidators(): void {
        ...
        const selectedBranchesControl =
            this.BillPrintForm.get('selectedBranches');
    ...
    private async fetchBanks() {
        try {
            ...
            if (
                ...
                response.result &&
                Array.isArray(response.result.banks)
            ) {
                // Map the banks to the desired format
                this.banks = response.result.banks.map(
    ...
    async onChangeBank(event: any) {
        if (event.value && event.value.code) {
            ...
            const response = await firstValueFrom(
                this.bankService.getBranchesByBankId(event.value.code).pipe(
                    tap((res) => {
                        if (res.result) {
    ...
    async onGenerate(generation: string) {
        if (this.BillPrintForm.valid) {
            ...
            const selectedMonth = this.months.find((m) => m.value.id === month)
                ?.value.name;
            ...
            try {
                ...
                switch (formValue.choices) {
                    ...
                    case 'allBankSpecificCategory':
                        this.generateAllBankSpecificCategoryReport(
                            year,
                            month,
                            this.categoryCode,
                            selectedMonth,
                    ...
                    case 'specificBackAllCategory':
                        this.generateSpecificBankAllCategoryReport(
                            year,
                            month,
                            formValue.bank.value.code,
                            selectedMonth,
                            isAllBank,
                            isAllCategory
                        );
                    ...
                    case 'specificBankSpecificCategory':
                        this.generateSpecificBankSpecificCategoryReport(
                            year,
                            month,
                            formValue.bank.value.code,
                            this.categoryCode,
                    ...
                    case 'specificBranchOrBranches':
                        const selectedBranchIds =
                            formValue.selectedBranches.map(
                        ...
                        await this.generateReportForSelectedBranches(
                            year,
                            month,
                            selectedBranchIds,
                            selectedMonth
    ...
    private async generateReportForSelectedBranches(
        year: number,
        month: number,
        branchIds: number[],
        ...
        try {
            ...
            this.processBillsAndGeneratePdf(
                response.result,
                year,
                selectedMonth,
                false,
            ...
            console.error(
                'Error generating report for selected branches:',
                error
            );
            this.toastService.showError(
                'Error generating report for selected branches'
    ...
    private async generateAllReport(
        year: number,
        month: number,
        selectedMonth: string,
        isAllBank: boolean,
    ...
    private async generateAllBankSpecificCategoryReport(
        year: number,
        month: number,
        categoryId: number,
        selectedMonth: string,
    ...
    private async generateSpecificBankAllCategoryReport(
        year: number,
        month: number,
        bankId: number,
        selectedMonth: string,
    ...
    private async generateSpecificBankSpecificCategoryReport(
        year: number,
        month: number,
        bank: number,
        category: number,
    ...
    private async processBillsAndGeneratePdf(
        bills: any,
        year: number,
        selectedMonth: string,
        isAllBank: boolean,
        ...
        try {
            this.fileGeneration.generatePdf(
                bills,
                year,
                selectedMonth,
                isAllBank,
    ...
    showBankInput(): boolean {
        ...
        return (
            choices === 'specificBackAllCategory' ||
            choices === 'specificBankSpecificCategory'
        );
    }
    showBankForBranches(): boolean {
        return (
            this.BillPrintForm.get('choices')?.value ===
            'specificBranchOrBranches'
        );
    }
    showCategoryInput(): boolean {
        const choices = this.BillPrintForm.get('choices')?.value;
        return (
            choices === 'allBankSpecificCategory' ||
            choices === 'specificBankSpecificCategory'
        );
    }
}
