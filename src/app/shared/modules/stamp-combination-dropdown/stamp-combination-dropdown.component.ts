...
export class StampCombinationDropdownComponent implements OnInit {
    ...
    formatResultItem(item: any): any {
        ...
        return {
            combination: `${item.stampCombinationId} | Category: ${item.stampCategory1} | Description: ${item.description} | Denomination: ${item.denomination} | No of Labels per Sheet: ${item.noLabelPerSheet}`,
        };
    }
            ...
            .subscribe((response) => {
                if (response.apiResponseStatus == 1) {
                    ...
                    response.result.map((item: any) => {
                        this.CombinationTypeList.push(
                            this.formatResultItem(item)
                        );
                    });
                } else {
    ...
    onStampCombinationSelected() {
        const val = this.data.filter((item) => {
            return (
                item.stampCombinationId ==
                this.extractFirstNumber(
                    this.selectedCombinationType.combination
                )
            );
        });
