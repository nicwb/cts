...
export class PensionerStatusComponent implements OnInit, ControlValueAccessor {
    ...
    getData(): void {
        this.pensionerStatusService
            .getStatus(this.ppoId, this.statusFlag)
            .pipe(
            ...
            .subscribe((response) => {
                ...
                if (
                    response.result &&
                    response.result.statusFlag !== undefined
                ) {
                    this.selectedStatus =
                        response.result.statusFlag > 0 ? 1 : 0;
                } else {
    ...
    deleteStatus(): void {
        this.pensionerStatusService
            .deleteStatus(this.ppoId, this.statusFlag)
            .subscribe(
                (response) => {
                    console.log('Status deleted:', response);
                    this.toastService.showSuccess(
                        'Status updated to ' + this.notApprovedText
                    );
                    this.previousStatus = 0;
                },
                (error: HttpErrorResponse) => {
