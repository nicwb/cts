...
test.describe('First Pension Component', () => {
...
test('should validate form fields', async ({ page }) => {
  ...
  test('should generate PDF and show error toast if failed', async ({ page }) => {
    ...
    if (isErrorToast) {
      ...
      console.log('Error message:', toastMessage);
    } else {
  ...
  test('should generate PDF and handle errors appropriately', async ({ page }) => {
    ...
    if (toastClasses.includes('error') || (toastMessage && toastMessage.toLowerCase().includes('error'))) {
      console.log('PDF generation failed as expected with error:', toastMessage);
      ...
      try {
        ...
        console.error('Failed to generate or verify PDF:', error);
        throw error;
      }
    }
  });
